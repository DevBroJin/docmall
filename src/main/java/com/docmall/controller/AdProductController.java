...
public class AdProductController {
	...
	//			   2)외부라이브러리를 이용.(pom.xml). servlet-context.xml에서 MultipartFile
	...
	public String pro_insert(ProductVO vo, MultipartFile uploadFile, RedirectAttributes rttr) {
		...
		//1)파일업로드 작업. 미리선수작업 : FileUtils 작업
		String dataFolder = FileUtils.getDateFolder();
		String savedFileName = FileUtils.uploadFile(uploadPath, dataFolder, uploadFile);
	...
	public void imageUpload(HttpServletRequest request, HttpServletResponse response, MultipartFile upload) {
		...
		try {
			...
			//1) 파일 업로드 작업
			String fileName = upload.getOriginalFilename(); // 클라이언트 전송 파일이름
