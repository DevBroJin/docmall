...
public class MemberController {
	...
	// 비동기방식. ajax문법으로 호출
	// 아이디 중복체크
	// ResponseEntity클래스? httpentity를 상속받는, 결과 데이터와 HTTP 상태 코드를 직접 제어할 수 있는 클래스.
	...
	public String login(LoginDTO dto, HttpSession session, RedirectAttributes rttr) {
	    ...
	    if (db_vo != null) {
	        // 사용자가 입력한 비밀번호 (평문 텍스트와)와 DB에서 가져온 암호화된 비밀번호 일치 여부 검사
	        if (passwordEncoder.matches(dto.getMbsp_password(), db_vo.getMbsp_password())) {
	        	// 로그인 성송결과로 서버측에 메모리를 사용하는 세션형태 작업
